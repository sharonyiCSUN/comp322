				...
				//for(int i = 2; i < argc; i++){
					// char ASCIIstr[8];
					 ...
					 for(int i = 2; i < argc; i++){
						 ...
						 int sumoforiginal1 = 0;
						 for(int k = 0; k < 8; k++){
                                                	 ASCIIstr[k] = copiedstr1[k+1];
                                                         sumoforiginal1 += (int)copiedstr1[k];
                                                                                                                                                                                                   }
                                                         char *ASCIIptr;
                                                         ...
                                                         char lastVal[] = "     DEL";
                                                         if(decimal < 10)
                                                	         printf("%s %s        %i ", copiedstr1, charVal[decimal], decimal);
							 if((decimal > 9) && (decimal < 33))
                                                                 printf("%s %s       %i ", copiedstr1, charVal[decimal], decimal);
							 if((decimal > 32) && (decimal < 100))
								 printf("%s        %c       %i ", copiedstr1, ASCIIchar, decimal);
                                        		 if((decimal > 101) && (decimal <127) )
					                         printf("%s        %c      %i ", copiedstr1, ASCIIchar, decimal);
				                         if(decimal == 127)
							         printf("%s %s      %i ", copiedstr1, lastVal, decimal);
	  	                                         if(sumoforiginal1%2 == 0)
							         printf("EVEN\n");
							 else
							         printf("ODD\n");
							 }
